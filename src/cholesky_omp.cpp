    ...
    #pragma omp parallel
    {
        #pragma omp single
        {
            for (int i = 0; i < num_blocks; i++) {
                ...
                // Cholesky任务
                #pragma omp task firstprivate(i, ib, b, n) shared(S, L, error_flag)
                {
                ...
                for (int j = i + 1; j < num_blocks; j++) {
                    ...
                    #pragma omp task firstprivate(i, j, ib, jb, b, n) shared(S, L)
                    {
                        trsm(&S[j * b * n + i * b], &L[i * b * n + i * b],
                             &L[j * b * n + i * b], jb, ib, n);
                    }
                }
                ...
                for (int j = i + 1; j < num_blocks; j++) {
                    ...
                    for (int k = i + 1; k <= j; k++) {
                        ...
                        #pragma omp task firstprivate(i, j, k, ib, jb, kb, b, n) shared(S, L)
                        {
                            madd(&L[j * b * n + i * b], &L[k * b * n + i * b],
                                 &S[j * b * n + k * b], jb, kb, ib, n);
                        }
                    }
                }
...
int main(int argc, char* argv[]) {
    ...
    // 测试并行版本 (简化版)
    printf("\n=== Parallel Block Cholesky (Simple) ===\n");
